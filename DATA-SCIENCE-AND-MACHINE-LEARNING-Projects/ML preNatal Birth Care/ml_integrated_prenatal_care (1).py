...
def random_forest(df, test_size=0.2, if_pca=False, n_components=10):
    X_train, X_test, y_train, y_test = get_train_test_data(df, if_pca, n_components, test_size)
    model_train_scores = dict()
    model_test_scores = dict()
    ...
    model.fit(X_train, y_train)
    model_train_scores['Random Forest with Pruning'] = model.score(X_train, y_train)
    model_test_scores['Random Forest with Pruning'] = model.score(X_test, y_test)

    print('Random Forest with Pruning')
    print('Train Score:', model_train_scores['Random Forest with Pruning'])
    print('Test Score:', model_test_scores['Random Forest with Pruning'])
    print('Best Parameters:', model.best_params_)
    print('Confusion Matrix:\n', confusion_matrix(y_test, model.predict(X_test)))
    print('Classification Report:\n', classification_report(y_test, model.predict(X_test)))
    print('--------------------------------------------------\n')
...
model_train_scores, model_test_scores = random_forest(preprocessed_df, test_size=0.2, if_pca=False, n_components=10)
...
def xgb_classifier(df, test_size=0.2, if_pca=False, n_components=10):
    X_train, X_test, y_train, y_test = get_train_test_data(df, if_pca, n_components, test_size)
    model_train_scores = dict()
    model_test_scores = dict()
    ...
    model = XGBClassifier()
    model.fit(X_train, y_train)
    model_train_scores['XGB Classifier'] = model.score(X_train, y_train)
    model_test_scores['XGB Classifier'] = model.score(X_test, y_test)

    print('XGB Classifier')
    print('Train Score:', model_train_scores['XGB Classifier'])
    print('Test Score:', model_test_scores['XGB Classifier'])
    print('Confusion Matrix:\n', confusion_matrix(y_test, model.predict(X_test)))
    print('Classification Report:\n', classification_report(y_test, model.predict(X_test)))
    print('--------------------------------------------------\n')

    # XGB Classifier with Pruning
    ...
    model.fit(X_train, y_train)
    model_train_scores['XGB Classifier with Pruning'] = model.score(X_train, y_train)
    model_test_scores['XGB Classifier with Pruning'] = model.score(X_test, y_test)

    print('XGB Classifier with Pruning')
    print('Train Score:', model_train_scores['XGB Classifier with Pruning'])
    print('Test Score:', model_test_scores['XGB Classifier with Pruning'])
    print('Best Parameters:', model.best_params_)
    print('Confusion Matrix:\n', confusion_matrix(y_test, model.predict(X_test)))
...
# Calling xgb_classifier
model_train_scores, model_test_scores = xgb_classifier(preprocessed_df, test_size=0.2, if_pca=False, n_components=10)

"""### **Observations**

* We can observe, that with 27 features, the data overfits the model and gives an   accuracy of ~99% even with a very low training set.
* Once we implement PCA, and select the most deciding components/features, it helps us in reducing the overfitting.
* Here, we are also using regularization, indicated by *solver = 'liblinear'*, which is L2, L1 regularization.
* Thus, we can observe that using these techniques, we successfully reduce the model overfitting and make it closer to real-world prediction scenario.
